...
def aggregate_cv_results(model_name: str, estimator_label: str, fallback_note: str, fold_metrics: list[dict[str, float]]) -> dict[str, float | str]:
...
def main() -> None:
    ...
    for spec in model_specs:
        ...
        cv_summary_row = aggregate_cv_results(model_name, estimator_label, fallback_note, fold_metrics)
    ...
    metadata = {
        ...
        "xgboost_available": best_estimator_label == "XGBClassifier",
        "fallback_note": fallback_note,
        "test_metrics": best_test_row,
        "feature_type_plan_path": str(FEATURE_TYPE_PLAN_PATH.relative_to(ARTIFACTS_DIR.parents[1])),
        "split_indices_path": str(SPLIT_INDICES_PATH.relative_to(ARTIFACTS_DIR.parents[1])),
    }
